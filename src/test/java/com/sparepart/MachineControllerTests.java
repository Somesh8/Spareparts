...
class MachineControllerTests {
	...
	void testGetAllMachines() throws Exception {
		when(machineService.getAllMachines())
				.thenReturn(Stream.of(new Machine(1, "Machine_name", "Machine_desc", mt, null),
		...
		mockMvc.perform(get("/machine").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$[0].machine_name", is("Machine_name")))
				.andExpect(jsonPath("$[0].machine_desc", is("Machine_desc")))
    ...
    public void getMachineTest() throws Exception {
		Machine machine1 = new Machine(1, "HP Pavilion 15", "Laptop", mt, null);
        
		when(machineService.getMachine(1)).thenReturn(machine1);

        mockMvc.perform(get("/machine/1").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.machine_id", is(1)))
                .andExpect(jsonPath("$.machine_name", is("HP Pavilion 15")))
	...
	void testSaveMachine() throws Exception {
		...
		MachineDTO newMachineDto = new MachineDTO(1, newMachine.getMachineName(), newMachine.getMachineDesc(), mt.getMachineTypeId(), company.getCompanyId());
	...
	void testUpdateMachine() throws Exception {
		...
		ObjectMapper objectMapper = new ObjectMapper();
        String json = objectMapper.writeValueAsString(newMachine);
        
        when(machineService.updateMachine(newMachine, 1)).thenReturn(newMachine); 
		
        mockMvc.perform(put("/machine/{id}",1)
        		.contentType(MediaType.APPLICATION_JSON)
                .content(json)
                .characterEncoding("utf-8"))
                .andExpect(status().is2xxSuccessful());
	...
	void testDeleteMachine() throws Exception {
		mockMvc.perform(delete("/machine/{id}",1).accept(MediaType.APPLICATION_JSON)
	            .contentType(MediaType.APPLICATION_JSON))
	            .andExpect(status().isOk());
	}
