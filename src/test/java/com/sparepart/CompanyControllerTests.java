...
class CompanyControllerTests {
	...
	public void getAllCompany() throws Exception {
		...
		when(cmpService.getAllCompanies()).thenReturn(companies);

		mockMvc.perform(get("/company").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(jsonPath("$[0].company_id", is(1))).andExpect(jsonPath("$[0].company_name", is("Comp_name")))
				.andExpect(jsonPath("$[0].company_desc", is("Comp_desc"))).andExpect(jsonPath("$.length()", is(2)));
	}
	...
	public void getAllCompanysTest() throws Exception {
		...
		mockMvc.perform(get("/company/1").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(jsonPath("$.company_id", is(1))).andExpect(jsonPath("$.company_name", is("HP Inc")))
	...
	public void testCompanyDelete() throws Exception {
		mockMvc.perform(
				delete("/company/{id}", 11).accept(MediaType.APPLICATION_JSON).contentType(MediaType.APPLICATION_JSON))
	...
	public void testSaveCompany() throws Exception {
		...
		mockMvc.perform(
				post("/company").contentType(MediaType.APPLICATION_JSON).content(json).characterEncoding("utf-8"))
	...
	public void testUpdateCompany() throws Exception {
		...
		mockMvc.perform(put("/company/{id}", 11).contentType(MediaType.APPLICATION_JSON).content(json)
